...
- **The detail page's two actions are icon buttons beside the item name** — ✏️ and 🗑️, in the delete red the swipe actions already use, on the `IconButton` style the list pages already use for ➕. They replace the "Delete" toolbar text and the full-width "Edit" bottom bar, and the bottom bar is gone rather than left holding one small icon. VOL-33's reason for keeping Delete out of that bar still stands: the edit form's Cancel and Save occupy both bottom slots and either one pops straight back to this page, so a button down there arrives under the finger that just tapped. Beside the name neither shares a position with anything, and the page gets back the strip the bar was using
...
- **Delete sits in the nav toolbar, not the bottom bar — found on the emulator, not in review.** The first cut put Edit and Delete side by side at the bottom, which placed Delete at the exact screen coordinates of the edit form's Save button. Saving pops straight back to this page, so the page arrives under the user's finger and a double tap on Save landed on Delete, with only the confirmation dialog between a fat finger and a deleted item — reintroducing the stray-tap deletion this issue set out to remove. Edit is now the only action in the bottom bar, full width, and Delete is a `ToolbarItem` sharing its position with nothing. Verified: a double tap on Save is now inert.
  **The toolbar placement is since superseded by VOL-31**, which made both actions icon buttons beside the item name — a different place, held for the same reason: nothing that deletes goes in the bottom bar
